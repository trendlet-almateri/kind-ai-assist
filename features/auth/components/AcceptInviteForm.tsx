 ...
 *
 *  PKCE flow (preferred, future):
 *    /auth/callback?token_hash=…&type=invite → server-side exchange → /accept-invite
 *    Session already active — just collect + set password.
 *
 *  Implicit flow (current Supabase default):
 *    /auth/callback#access_token=…&type=invite → lands here with hash
 *    We extract access_token + refresh_token from the hash and call setSession().
 */
...
type Step = 'init' | 'set-password' | 'completing' | 'error'
...
export function AcceptInviteForm() {
  ...
  // ── On mount: detect which flow we're in ────────────────────────────────
  useEffect(() => {
        ...
        supabase.auth
          ...
          .then(({ error: err }) => {
            if (err) {
              ...
              // Clean the hash from the URL so the tokens aren't visible
    ...
    // ── PKCE flow: session already active (set by /auth/callback route) ──
    // Just go straight to the password form.
    setStep('set-password')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Server action: set role cookies + redirect to inbox/dashboard
    setStep('completing')
      ...
      >
        ...
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {step === 'init'         && 'Verifying your invite…'}
        ...
        {/* Spinner while verifying or completing */}
        {(step === 'init' || step === 'completing') && (
